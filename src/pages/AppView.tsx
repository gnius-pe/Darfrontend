...
const AppView: React.FC = () => {
  ...
  if (!data) {
    return <div>Loading...</div>; // Manejo del estado de carga
  }
    ...
    <div className="h-auto flex flex-col justify-center bg-custom-fondodash">
      ...
      <div className="mt-4 space-y-3 md:space-y-0 grid grid-cols-1 gap:2 lg:gap-4 md:grid-cols-3 p-2 sm:p-0 md:grid-rows-5 max-w-[1070px] mx-auto">
        <div className="row-span-2 md:row-span-2">
          <Cupos quota={data.quota} />
        </div>
        <div className='row-span-2 md:row-span-2 space-y-3'>
          <CountCard title="Total de inscritos" countPacient={data.allInscribe} imageSrc={ic_world} bgColor='bg-custom-blue' />
          <CountCard title="Niños inscritos" countPacient={data.patientChildren} imageSrc={ic_human} bgColor='bg-teal-300' />
        </div>
