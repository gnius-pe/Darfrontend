...
const CuposDisponibles: React.FC = () => {
  ...
  useEffect(() => {
    ...
    axios.get<Specialty[]>(import.meta.env.VITE_API_DASHESPEC)
      .then(response => {
        ...
        const specialtiesWithColors = response.data.map((specialty, index) => ({
          ...
          initial: specialty.Especialidad.charAt(0).toUpperCase(),
          color: colors[index % colors.length],
          disponibles: specialty.disponibles,
          atendidos: specialty.atendidos,
          consulta: specialty.consulta,
          pendientes: specialty.pendientes
        }));
      ...
      <div className="mx-auto ">
        ...
        <table className="text-center bg-white rounded-lg min-w-full">
          ...
          {specialties.map((specialty) => {
                    ...
                    const totalCupos = specialty.disponibles + specialty.atendidos + specialty.consulta + specialty.pendientes;
                    return (
                      <tr className='py-2' key={specialty.id}>
                        ...
                        <td className='h-12 flex'>
                          <div className='flex items-center gap-3'>
                            ...
                            <h2>{specialty.Especialidad}</h2>
                          </div>
                        </td>
                        <td>{specialty.disponibles}</td>
                        <td>{specialty.atendidos}</td>
                        <td>{specialty.consulta}</td>
                        <td>{specialty.pendientes}</td>
                        <td>{totalCupos}</td> {/* Muestra la suma aquí */}
                        <td>
